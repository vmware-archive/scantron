...
RSpec.describe Scantron::Scanner do
  ...
  context 'when the inventory machine is in the results' do
    ...
    before do
      allow(host).to receive(:each_port).and_yield(service1).and_yield(service2).and_yield(app_port).and_yield(unknown_port)
      allow(parser).to receive(:parse).with('service1-output').and_return('service1-name')
      allow(parser).to receive(:parse).with('service2-output').and_return('service2-name')
      allow(parser).to receive(:parse).with('app_port-output').and_raise(Scantron::NoServicesListeningOnPort)
      allow(parser).to receive(:parse).with('unknown_port-output').and_raise(Scantron::NoServicesListeningOnPort)
